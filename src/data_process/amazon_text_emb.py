    ...
    with torch.no_grad():
        while start < len(order_texts):
            ...
            for sentences in field_texts:
                ...
                input_device = torch.device(f"cuda:{model.hf_device_map['embed_tokens']}")
                # print("input_device: ", input_device)
                input_ids = encoded_sentences['input_ids'].to(input_device)
                attention_mask = encoded_sentences['attention_mask'].to(input_device)
...
if __name__ == '__main__':
    ...
    # kwargs = {"cache_dir": args.model_cache_dir, "local_files_only": False}
    ...
    print(f"Before loading model: {torch.cuda.memory_allocated()} bytes")
    
    # # Using device when single GPU
    # plm_model = plm_model.to(device)

    print(f"After loading model: {torch.cuda.memory_allocated()} bytes")
