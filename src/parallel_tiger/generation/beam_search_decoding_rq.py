...
def _get_valid_mask(
    ...
    if t in transition_masks and transition_masks[t] is not None:
        ...
        prefix_to_uidx_t3 = prefix_to_uidx_t3.to(device)
        uidx_to_next_tokens_t3 = uidx_to_next_tokens_t3.to(device)
...
class CustomGeneration:
    def __init__(
        self, 
        model: 'RQQTransformer', # type: ignore[reportMissingTypeStubs]
        use_multi_head: bool, 
        mask_token_id: int, 
        stochastic: bool,
        ...
        self.temperatures = temperatures if temperatures else [1.0] * model.depth_seq_len
    ...
    def _get_model_properties(self, logits: torch.Tensor) -> Tuple[int, int, torch.device, int, int]:
        ...
        code_num = self.model.num_tokens
        no_special_tokenizer_tokens = self.model.num_special_tokens
    ...
    def _get_constraints(self, use_constraints: bool, device: torch.device, code_num: int) -> Tuple[torch.Tensor, dict[int, torch.Tensor] | dict[int, None]]:
        ...
        if use_constraints:
            assert self.model.first_token_constraint_mask is not None, \
                "First token constraints not set. Use first_token_constraint_mask."
            assert self.model.transition_constraint_masks is not None, \
                "Transition constraints not set. Use set_transition_constraint_masks."

            first_token_constraints = self.model.first_token_constraint_mask.to(device)
            transition_masks = self.model.transition_constraint_masks
            transition_masks = {t: mask.to_dense().to(device) for t, mask in transition_masks.items() if mask is not None}
        else:
    ...
    def _initialize_generation(
        ...
        # logits0: (bs, code_num)
        valid_logits0 = logits0.masked_fill(~first_token_constraints, float("-inf"))
    ...
    def generate(self, logits: torch.Tensor, topK: int, use_constraints: bool) -> torch.Tensor:
